...
router.get('/', async (ctx) => {
    ...
    //获取数据总的条数
    let countData = await DB.count('link',{});
    //console.log(countData);
    let page = ctx.query.page || 1; //获取当前页数
    let pageSize = 3;

    let data = await DB.find('link',{},{},{
        page,
        pageSize,
        sort:{
            add_time:-1
...
router.get('/edit', async (ctx) => {
    ...
    //查找数据库
    let data = await DB.find('link',{'_id':DB.getObjectID(ctx.query.id)});
    if(data.length>0){
...
router.post('/doEdit',tools.multer().single('pic'),async (ctx)=>{
    ...
    let result = await DB.update('link',{'_id':DB.getObjectID(json.id)},json);
